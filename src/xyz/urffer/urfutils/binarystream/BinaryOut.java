package xyz.urffer.urfutils.binarystream;

/* this code copied directly from princeton's standard library
 ...
 * <p>
 * The client should not intermixing calls to {@code BinaryOut} with calls to
...
public final class BinaryOut {
	...
	/**
	 * Closes and flushes the binary output stream. Once it is closed, bits can
	 ...
	 *             unless {@code r} is between 1 and 32
	 * @throws IllegalArgumentException
	 *             unless {@code x} is between 0 and 2<sup>r</sup> - 1
	 */
	public void writeInt(int x, int r) {
		if (r == 32) {
			writeInt(x);
			return;
		}
	 ...
	 * @throws IllegalArgumentException
	 *             unless {@code x} is betwen 0 and 255
	 */
	 ...
	 *            the number of relevant bits in the char
	 * @throws IllegalArgumentException
	 *             unless {@code r} is between 1 and 16
	 * @throws IllegalArgumentException
	 *             unless {@code x} is between 0 and 2<sup>r</sup> - 1
	 */
	public void writeChar(char x, int r) {
		if (r == 8) {
			writeChar(x);
			return;
		}
	 ...
	 * @throws IllegalArgumentException
	 *             unless r is between 1 and 16
	 * @throws IllegalArgumentException
	 *             if any character in the string is not between 0 and
	 *             2<sup>r</sup> - 1
	 */
	public void writeString(String s, int r) {
		for (int i = 0; i < s.length(); i++)
			writeChar(s.charAt(i), r);
	}
